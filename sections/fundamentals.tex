...
The LASSO platform employs \emph{Stimulus-Response Matrices} as a unified, language-independent representation for capturing test execution data~\cite{Kessel2022}. An SRM is formally defined as a binary matrix $M \in \{0,1\}^{n \times m}$, where $n$ represents the number of test cases and $m$ denotes the number of coverage requirements. Each matrix element $M_{ij}$ encodes the coverage relationship between test case $i$ and requirement $j$:
...
\textbf{Example:} Consider a simple SRM with 4 test cases and 6 requirements:
\begin{equation}
M = \begin{pmatrix}
1 & 1 & 0 & 1 & 0 & 0 \\
1 & 0 & 1 & 0 & 1 & 0 \\
